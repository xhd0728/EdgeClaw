...
function createSettings(overrides: Partial<IndexingSettings> = {}): IndexingSettings {
  return {
...
function createL0(overrides: Partial<L0SessionRecord> = {}): L0SessionRecord {
  return {
    ...
    messages: [
      ...
      {
        role: "assistant",
        content: "We switched to deterministic candidates plus evidence notes.",
      },
    ],
    source: "openclaw",
    indexed: true,
...
function createExtractor(overrides: Record<string, unknown> = {}) {
  return {
    decideMemoryLookup: vi.fn().mockResolvedValue({
      ...
      baseOnly: false,
      lookupQueries: [
        { targetTypes: ["time", "project"], lookupQuery: "default", timeRange: null },
      ],
    }),
...
function emitPromptDebug(
  input: { debugTrace?: (debug: Record<string, unknown>) => void } | undefined,
  requestLabel: string,
...
describe("ReasoningRetriever", () => {
  ...
  it("passes recent messages into hop1 and uses effectiveQuery after hop1", async () => {
    const time = createL2Time({
      dateKey: "2026-04-02",
      summary: "Handled several work items in Xibeiwang.",
    });
    const repository = createRepository({
    ...
    const extractor = createExtractor({
      decideMemoryLookup: vi
        .fn()
        .mockImplementation(
          async (input: { recentMessages?: Array<{ role: string; content: string }> }) => {
            ...
            return {
              ...
              baseOnly: false,
              lookupQueries: [
                { targetTypes: ["time"], lookupQuery: "西北旺 做了什么", timeRange: null },
              ],
            };
          },
        ),
    ...
    expect(repository.searchL2TimeIndexes as ReturnType<typeof vi.fn>).toHaveBeenCalledWith(
      "西北旺 做了什么",
      10,
    );
    expect(selectL2FromCatalog).toHaveBeenCalledWith(
  ...
  it("uses direct date-key reads for time-range retrieval", async () => {
    ...
    const repository = createRepository({
      getL2TimeByDate: vi
        .fn()
        .mockImplementation((dateKey: string) => (dateKey === "2026-03-31" ? time : undefined)),
      searchL2TimeIndexes: vi.fn().mockReturnValue([]),
    });
    const extractor = createExtractor({
      decideMemoryLookup: vi.fn().mockResolvedValue({
        ...
        lookupQueries: [
          {
            targetTypes: ["time"],
            lookupQuery: "今天做了什么",
            timeRange: { startDate: "2026-03-31", endDate: "2026-03-31" },
          },
        ],
      }),
      selectL2FromCatalog: vi.fn().mockResolvedValue({
    ...
    expect(repository.getL2TimeByDate as ReturnType<typeof vi.fn>).toHaveBeenCalledWith(
      "2026-03-31",
    );
  ...
  it("uses BM25-style project retrieval for project-only queries", async () => {
    ...
    const extractor = createExtractor({
      decideMemoryLookup: vi.fn().mockResolvedValue({
        ...
        baseOnly: false,
        lookupQueries: [
          { targetTypes: ["project"], lookupQuery: "ClawXMemory 进展", timeRange: null },
        ],
      }),
      selectL2FromCatalog: vi.fn().mockResolvedValue({
        intent: "project",
        evidenceNote:
          "ClawXMemory is currently refactoring retrieval and has implemented deterministic L2 routing.",
        enoughAt: "l2",
      }),
    });
    ...
    expect(repository.searchL2ProjectIndexes as ReturnType<typeof vi.fn>).toHaveBeenCalledWith(
      "ClawXMemory 进展",
      10,
    );
  });
  ...
  it("keeps time hits ahead of project hits for mixed queries", async () => {
    ...
    const repository = createRepository({
      ...
      searchL2ProjectIndexes: vi
        .fn()
        .mockReturnValue([{ level: "l2_project", score: 0.9, item: project }]),
    });
    const extractor = createExtractor({
      decideMemoryLookup: vi.fn().mockResolvedValue({
        memoryRelevant: true,
    ...
    const result = await retriever.retrieve("我今天这个项目进展如何", {
      retrievalMode: "explicit",
      l2Limit: 4,
    });
  ...
  it("descends to L1 and L0 in accuracy_first mode and dedupes/sorts candidates", async () => {
    ...
    const l1C = createL1({
      l1IndexId: "l1-3",
      endedAt: "2026-03-31T08:00:00.000Z",
      l0Source: ["l0-4"],
    });
    ...
    const repository = createRepository({
      ...
      getL1ByIds: vi
        ...
        .mockImplementation((ids: string[]) =>
          [l1A, l1B, l1C].filter((item) => ids.includes(item.l1IndexId)),
        ),
      getL0ByIds: vi
        .fn()
        .mockImplementation((ids: string[]) =>
          [l0A, l0B, l0C].filter((item) => ids.includes(item.l0IndexId)),
        ),
    });
    const extractor = createExtractor({
      decideMemoryLookup: vi
        .fn()
        .mockImplementation(
          async (input: { debugTrace?: (debug: Record<string, unknown>) => void }) => {
            const parsed = {
              memoryRelevant: true,
              baseOnly: false,
              lookupQueries: [
                {
                  targetTypes: ["time", "project"],
                  lookupQuery: "最近一周 ClawXMemory 进展",
                  timeRange: { startDate: "2026-03-31", endDate: "2026-03-31" },
                },
              ],
            };
    ...
    expect(
      result.trace?.steps.find((step) => step.kind === "hop2_decision")?.details?.length,
    ).toBeGreaterThan(0);
    expect(
      result.trace?.steps.find((step) => step.kind === "hop4_decision")?.promptDebug,
    ).toMatchObject({
  ...
  it("continues descending when hop2 and hop3 return none in accuracy_first mode", async () => {
    ...
    const repository = createRepository({
      ...
      getL1ByIds: vi
        .fn()
        .mockImplementation((ids: string[]) => [l1].filter((item) => ids.includes(item.l1IndexId))),
      getL0ByIds: vi
        .fn()
        .mockImplementation((ids: string[]) => [l0].filter((item) => ids.includes(item.l0IndexId))),
    });
    const extractor = createExtractor({
      decideMemoryLookup: vi.fn().mockResolvedValue({
        memoryRelevant: true,
