...
describe("MemoryRepository L2 search", () => {
  ...
  it("keeps project-only search from being displaced by time hits", async () => {
    ...
    repository.upsertL2TimeIndex(createTimeIndex({ l2IndexId: "l2-time-1", dateKey: "2026-03-31" }));
    repository.upsertL2TimeIndex(createTimeIndex({ l2IndexId: "l2-time-2", dateKey: "2026-03-30", summary: "Alpha work continued yesterday." }));
  ...
  it("keeps time-only search from being displaced by project hits", async () => {
    const repository = await createRepository();
    repository.upsertL2ProjectIndex(createProjectIndex({ l2IndexId: "l2-project-1", projectName: "Release Board" }));
    repository.upsertL2ProjectIndex(createProjectIndex({
    ...
    repository.upsertL2TimeIndex(createTimeIndex({
      l2IndexId: "l2-time-3",
      dateKey: "2026-03-29",
  ...
  it("merges per-type top-k results for mixed L2 search", async () => {
    const repository = await createRepository();
    repository.upsertL2TimeIndex(createTimeIndex({ l2IndexId: "l2-time-mixed", summary: "Beta work happened today." }));
    repository.upsertL2ProjectIndex(createProjectIndex({
