...
describe("plugin entry", () => {
  ...
  it("registers memory prompt section, tools, hooks, and service", async () => {
    ...
    const startSpy = vi.spyOn(MemoryPluginRuntime.prototype, "start");
    const stopSpy = vi.spyOn(MemoryPluginRuntime.prototype, "stop");
    ...
    const memoryRuntime = registerMemoryRuntime.mock.calls[0]?.[0] as {
      getMemorySearchManager: (params: {
        ...
        agentId: string;
      }) => Promise<{ manager: null; error: string }>;
      resolveMemoryBackendConfig: (params: { cfg: Record<string, unknown>; agentId: string }) => {
        backend: "builtin";
      };
    };
    expect(service).toMatchObject({ id: "openbmb-clawxmemory-runtime" });
    expect(memoryRuntime.resolveMemoryBackendConfig({ cfg: {}, agentId: "main" })).toEqual({
      backend: "builtin",
    });
    await expect(
      memoryRuntime.getMemorySearchManager({ cfg: {}, agentId: "main" }),
    ).resolves.toEqual({
      manager: null,
      error:
