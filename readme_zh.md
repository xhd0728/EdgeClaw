...
- **[2026.04.03]** 🔧 [ClawXTool](./extensions/clawxtool/) 发布 — 8 合 1 统一工具套件（cron 解析、bash 安全分析、密钥扫描、Notebook 编辑、Git Worktree、任务管理、记忆时效标注、结构化用户提问）
- **[2026.04.03]** 🔍 [ClawXSkill](./extensions/clawxskill/) 发布 — 智能 Skill 发现引擎，支持 BM25 关键词搜索、可选 embedding 语义搜索、LLM 模型裁判
- **[2026.04.03]** 🧠 [ClawXMemory](./extensions/openbmb-clawxmemory/) 升级至 v0.1.5 — 重构检索推理链路，改进 dream-review，新增 message-utils 与 prompt-section 模块
...
- **🔧 统一工具套件** — [ClawXTool](./extensions/clawxtool/)：8 合 1 插件提供 13 个工具——cron 解析、bash 安全分析、密钥扫描、Notebook 编辑、Git Worktree 管理、结构化任务追踪、记忆时效标注、交互式用户提问
