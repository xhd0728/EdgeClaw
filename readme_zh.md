...
- **[2026.04.03]** 🚀 三大 Claude Code liked 特性齐发：🔧 [ClawXTool](./extensions/clawxtool/) 推出 8 合 1 工具套件（含安全分析、密钥扫描、Git Worktree等），🔍 [ClawXSkill](./extensions/clawxskill/) 发布智能发现引擎（支持skill搜索与模型裁判），🧠 [ClawXContext](./extensions/openbmb-clawxcontext/) 引入长会话平滑压缩与动态上下文重入机制。
...
- **🔧 统一工具套件** — [ClawXTool](./extensions/clawxtool/)：8 合 1 插件提供 13 个工具，涵盖**安全审计**（bash 安全分析、密钥扫描）、**工作流**（Git Worktree 管理、结构化任务追踪）、**开发辅助**（cron 解析、Notebook 编辑）与 **Agent 交互**（记忆时效标注、交互式用户提问）
- **🔍 Skill 发现** — [ClawXSkill](./extensions/clawxskill/)：自动发现并索引工作区中的 Agent Skills，支持 BM25 关键词搜索、可选 embedding 语义搜索、LLM 模型裁判智能匹配
- **🧠 记忆引擎** — [ClawXMemory](./extensions/openbmb-clawxmemory/)：面向 OpenClaw 打造的结构化长期记忆引擎，在借鉴 Claude Code 记忆机制思路的基础上，进一步构建了多层结构化记忆与模型驱动的记忆检索机制 _(v0.1.5)_
- **📝 上下文引擎** — [ClawXContext](./extensions/openbmb-clawxcontext/)：专注于长会话稳定性的 OpenClaw 上下文引擎，引入平滑的上下文压缩与动态重入机制。
- **🐾 虚拟宠物伴侣** — [ClawXBuddy](./extensions/clawxbuddy/)：一款可爱的 ASCII 虚拟宠物伴侣，支持待机动画、稀有度属性和交互指令，在编码时陪伴你。
...
│   │       └── config.ts             # 沙箱配置
│   │
│   ├── clawxtool/                       # [内置] ClawXTool 统一工具套件（8 合 1）
│   │   ├── index.ts                     # 统一入口（13 个工具 + hooks）
│   │   └── src/                         # 工具逻辑模块
│   │       ├── cron-parser.ts           # Cron 表达式解析
│   │       ├── classifier.ts            # Bash 命令安全分类
│   │       ├── secret-patterns.ts       # 密钥/令牌检测规则
│   │       ├── notebook.ts              # Jupyter Notebook 读写
│   │       ├── worktree.ts              # Git Worktree 管理
│   │       ├── store.ts                 # 任务持久化存储
│   │       ├── annotate.ts              # 记忆时效标注
│   │       └── question-tool.ts         # 结构化多选提问
│   │
│   ├── clawxskill/                      # [内置] ClawXSkill 智能 Skill 发现
│   │   ├── index.ts                     # 插件入口
│   │   ├── engines/
│   │   │   ├── inverted-index.ts        # BM25 关键词搜索引擎
│   │   │   ├── embedding-search.ts      # 语义 embedding 搜索
│   │   │   └── model-judge.ts           # LLM Skill 相关性裁判
│   │   ├── watcher.ts                   # 文件系统 Skill 监听
│   │   └── skill-backup.ts             # Skill 索引备份/恢复
│   │
│   ├── clawxbuddy/                      # [内置] ClawXBuddy 虚拟宠物伴侣
│   │   └── index.ts                     # 插件入口（ASCII 精灵与待机动画）
│   │
...
- [ClawXContext](https://github.com/OpenBMB/ClawXContext) — 面向长会话稳定性与平滑压缩的上下文引擎
...
- [ClawXTool](./extensions/clawxtool/) — 8 合 1 统一工具套件（cron、bash 安全、密钥扫描、Notebook、Git Worktree、任务管理、记忆时效、用户提问）
- [ClawXSkill](./extensions/clawxskill/) — 智能 Skill 发现引擎，支持 BM25 关键词搜索、embedding 语义搜索、LLM 模型裁判
- [ClawXBuddy](./extensions/clawxbuddy/) — 虚拟宠物伴侣，支持 ASCII 动画精灵、待机反应与基于稀有度的个性属性
