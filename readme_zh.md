...
- **[2026.04.03]** 🚀 三大 Claude Code liked 特性齐发：🔧 [ClawXTool](./extensions/clawxtool/) 推出 8 合 1 工具套件（含安全分析、密钥扫描、Git Worktree等），🔍 [ClawXSkill](./extensions/clawxskill/) 发布智能发现引擎（支持skill搜索与模型裁判），🧠 [ClawXMemory](./extensions/openbmb-clawxmemory/) v0.1.5 重构检索链路并改进 dream-review 机制。
...
- **🧠 记忆引擎** — [ClawXMemory](./extensions/openbmb-clawxmemory/)：面向 OpenClaw 打造的结构化长期记忆引擎，在借鉴 Claude Code 记忆机制思路的基础上，进一步构建了多层结构化记忆与模型驱动的记忆检索机制 *(v0.1.5)*
